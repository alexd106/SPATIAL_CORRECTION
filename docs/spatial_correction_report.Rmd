...
pkgs <- c("SpATS", "mgcv", "sommer", "ggplot2", "patchwork")
for (pkg in pkgs) {
...
This document describes the development and validation of a general approach for spatial correction in field/glasshouse-based trial data in R. Spatial variation across field plots or glasshouse benches, caused by irrigation gradients, soil heterogeneity, edge drying, and microclimate effects, may bias phenotypic measurements and thereby impact genotype rankings. Correcting for this variation is important before downstream analyses such as GWAS.

The report is structured in two parts:

**Part 1** compares three packages: SpATS, sommer, and mgcv on replicated real data. The aim here is to specify a generalised additive model using the package `mgcv` and compare performance with the existing SpATS and sommer packages. The GAM fitted using mgcv with default settings substantially under-corrects relative to SpATS and sommer. The performance gap is due to a single missing model component: row and column random effects. Adding these closes the gap entirely.

**Part 2** addresses the primary use case to correct phenotypes where genotypes are unreplicated within each field/plot/bench (hereafter referred to as a bench) but replicated across spatial units. Per-bench models (including SpATS) can only produce BLUPs (shrunken estimates) that compress genotype differences and are not particularly suitable for downstream GWAS. Joint multi-bench models recover proper BLUEs by pooling information across benches, reducing RMSE by approximately 3× relative to averaged per-bench BLUPs.
...
In a **replicated** design (multiple observations per genotype within a field/bench), genotype effects can be estimated as fixed effects producing BLUEs (Best Linear Unbiased Estimates). BLUEs are unshrunken estimates on the true phenotypic scale, the preferred input for GWAS.

### Model specifications

We compare three packages: SpATS and sommer, both developed specifically for spatial field trial analysis, and mgcv, a more general modelling package. All three share the same conceptual structure (estimating a 2D spatial surface and extracting genotype effects) but differ in how the surface is modelled and estimated.
...
mgcv fits a generalised additive model (GAM) with a tensor-product smooth for the spatial surface:
...
```r
gam(yield ~ 0 + Genotype + te(row, col), data = df, method = "REML")
```
...
We validated on the `wheatdata` dataset (comes with the SpATS package): 330 plots, 107 genotypes, 3 replicates, 22 rows × 15 columns.
...
We tested five mgcv variants to isolate the cause:
...
Row and column random effects capture systematic linear-like variation (irrigation gradients, edge drying etc) that a smooth 2D surface cannot efficiently represent on its own. SpATS and sommer include these by default.

When row and column random effects are included, the mgcv model achieves equivalent performance to both SpATS and sommer for replicated designs: residual standard deviations are near-identical (34.0 for mgcv vs 34.1 for SpATS and 33.2 for sommer) and genotype BLUEs correlate at r = 0.9996 with SpATS. This confirms that all three frameworks estimate the same underlying spatial structure and that the choice of package for replicated data is largely one of convenience.
...
Or if you prefer the R code version: 

```r
gam(yield ~ 0 + Genotype +
      te(row, col, bs = "ps") +
...
Our primary use case involves multiple benches (in our case 4) where genotypes are unreplicated within each bench but replicated across benches. This creates a fundamental statistical problem for per-bench correction.

To obtain BLUEs we need genotype as a fixed effect, which means estimating one parameter per genotype. With $n$ genotypes and only $n$ observations per bench (one per genotype), the genotype effects consume all available degrees of freedom. The model is saturated and there is nothing left for the spatial smooth to estimate against. In practice:
...
To validate the joint models against known truth, I wrote a little script `simulate_spatial_data.R` to generate multi-bench trial data with the following structure:
...
- **Spatial pattern** (edge effects): elevated at field borders with diagonal asymmetry (top-left high, bottom-right low), centred to mean zero. This mimics glasshouse bench effects: drying at exposed edges, irrigation run-off gradients, asymmetric light etc.
- **Varying spatial pattern intensity per bench**: 15, 25, 40, 60, ranging from moderate (intensity 15 ≈ ±6 units, half of genotype SD) to very high (intensity 60 ≈ ±23 units, nearly twice genotype SD of 12)
- Genotype SD = 12, error SD = 4

This is the known 'true' spatial pattern. In practice we would never know this but when validating a potential modelling approach it's useful.

```{r s2-true-spatial, fig.cap="**Figure 2.1:** True spatial patterns for each bench (edge effects with diagonal asymmetry). A single shared colour scale is used across all four panels to directly convey the 4-fold increase in spatial severity from Bench1 (intensity = 15) to Bench4 (intensity = 60, dominant yellow-to-purple contrast). The pattern mimics glasshouse edge effects: elevated phenotypes at borders, depressed in the interior."}
knitr::include_graphics("figures/s2_true_spatial.png")
```

### Per-bench SpATS BLUPs: the shrinkage problem

With unreplicated within-bench data, running SpATS for each bench separately is limited to BLUPs. The figure below shows that BLUPs are systematically compressed relative to the true genotype values with the slope against the 1:1 line visibly less than 1 in every bench:
...
We can do this using the existing sommer package (with caveats below) or extend our more general modelling approach using the `mgcv` package.
...
3. **Joint sommer BLUEs:** single joint mixed model, genotype as fixed effect
4. **Joint mgcv BLUEs:** single joint GAM, genotype as fixed effect
...
```{r s2-sommer-spatial, fig.cap="**Figure 2.3:** Estimated spatial trends from the joint sommer model (column-offset approach). Shared colour scale across panels (same scale as Figure 2.1). The overall patterns align with the true spatial structure (Figure 2.1). However, the magnitude of the estimated surfaces is systematically attenuated, particularly in benches with higher intensity. This attenuation is caused by B-spline boundary penalisation at the column edges and is the root cause of the BLUE upward shift described below."}
knitr::include_graphics("figures/s2_sommer_joint_spatial.png")
```

```{r s2-mgcv-spatial, fig.cap="**Figure 2.4:** Estimated spatial trends from the joint mgcv model. Shared colour scale across panels (same scale as Figure 2.1). Bench1 (intensity = 15) shows a moderate edge-effect pattern, correctly estimated with low spatial amplitude. Bench4 (intensity = 60) shows the strong edge-effect pattern, correctly recovered. The model adapts spatial complexity independently to each bench via the te(..., by=bench_f) term."}
knitr::include_graphics("figures/s2_mgcv_joint_spatial.png")
```

#### Scatter plots: estimates vs true genotype values

```{r s2-scatter, fig.cap="**Figure 2.5:** Estimated vs true genotype values for all four approaches. Per-bench SpATS BLUP averages (top right) show visible compression toward the mean: the cloud is narrower on the y-axis than on the x-axis. Uncorrected means (top left) show wider scatter caused by unremoved spatial bias. Joint sommer BLUEs (bottom left) correlate well with true values but show a systematic upward shift above the 1:1 line; all estimates are positively biased. Joint mgcv BLUEs (bottom right) cluster tightly around the 1:1 line across the full range of true values."}
knitr::include_graphics("figures/s2_scatter_estimates_vs_true.png")
```

#### Why sommer joint BLUEs are shifted above the 1:1 line

The scatter plot (Figure 2.5) shows that all sommer joint BLUEs are positively biased and sit consistently above the 1:1 reference line rather than scattered around it. This is not a model failure but an artefact of the column-offset trick interacting with the edge-effect spatial pattern.
...
This limitation is specific to the column-offset workaround. A native `vsm(dsm(bench), spl2Dc(...))` implementation would fit each bench's surface with proper column boundaries, eliminating the gap-induced penalty. Until sommer supports this syntax, joint mgcv (`te(..., by=bench_f)`) is the preferred approach for designs with strong edge spatial patterns.
...
Residual SD is consistent across all four benches (3.3–3.6) despite a 4-fold variation in spatial intensity (15 to 60 units). This confirms that both the column-offset `spl2Dc` and `te(..., by=bench_f)` fit genuinely independent surfaces that scale correctly from the moderate pattern of Bench1 to the strong edge effects of Bench4.

#### SpATS per-bench is limited to BLUPs in this design

SpATS cannot produce BLUEs from unreplicated within-bench data and does not natively support joint multi-bench models. For the glasshouse designs we typically use (unreplicated within bench, replicated across benches), sommer or mgcv joint models are required to obtain proper BLUEs.
...
knitr::kable(app_df, digits = c(0, 0, 4, 2),
             ...
             caption = "**Table A1:** Genotype recovery accuracy for row gradient, column gradient, and localised hotspot patterns. Joint mgcv BLUEs achieve substantially lower RMSE than per-bench SpATS BLUPs despite similar correlations, confirming that the BLUE/BLUP distinction (not spatial correction quality) drives the difference. Results are consistent across all three pattern types.")
```
